...
// specification/manufacturer/model live directly on the resource itself (NOT
// on resource.product) — the same product can be listed by different orgs
// under different manufacturers. `brand` moved OFF the resource and onto the
// product's own attributes (FRONTEND_AI_PROMPT_BRAND_IDENTITY.md) — it's now
// part of product identity, read via GET /api/products/{id}/attributes, not
// this type.
...
// missing/blank), `specification`/`model` optional — these are
...
// PUT /api/resources/{id} (§ 3.3) — `active` plus the listing fields (a
// vendor correcting their own manufacturer name). `productId`/category/
...
// manufacturer/unit/attributeName/attributeValue moved to
